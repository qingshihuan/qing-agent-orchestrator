...
function compare(actual, verification) {
...
function observe(events, verification) {
    ...
    if (!allowedFields[verification.eventType]?.has(verification.field))
        throw new Error("Relay payload field is not on the collector whitelist.");
...
export async function collectRelayCriterionEvidence(handoff, runHandle, iteration, clock = () => new Date().toISOString()) {
    ...
    try {
        ...
        const evidence = handoff.acceptanceCriteria.flatMap((criterion) => {
            if (criterion.verificationOwner === "executor")
                return [];
            const verification = criterion.relayVerification;
            if (!verification)
                throw new Error(`Criterion ${criterion.id} has no relayVerification.`);
