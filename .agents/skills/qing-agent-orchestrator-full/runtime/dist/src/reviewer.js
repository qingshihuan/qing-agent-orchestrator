...
export class RuleBasedReviewer {
    async review(handoff, execution, iteration, relayEvidence = [], trustedCommands) {
        relayEvidence = relayEvidence.filter((item) => item.iteration === iteration);
        const criterionAudit = handoff.acceptanceCriteria.map((criterion) => {
            ...
            if (executorFails || relayFails)
                status = "fail";
            else if (criterion.verificationOwner === "executor")
                status = executorPasses ? "pass" : "not_verified";
            else if (criterion.verificationOwner === "relay")
                status = relayPasses ? "pass" : "not_verified";
            else
                status = executorPasses && relayPasses ? "pass" : "not_verified";
            return {
                id: criterion.id,
        ...
        const criteria = criterionAudit.map(({ id, status, evidence }) => ({ id, status, evidence }));
        const tests = handoff.testPlan.map((command) => {
            if (trustedCommands !== undefined) {
                ...
                return { command, status: consistent ? "passed" : "failed", evidence: consistent ? `Relay parent process executed the exact declared command with exitCode 0 (${item.startedAt}..${item.endedAt}).` : `Relay parent test evidence count=${matches.length}; expected one complete, bound, successful record.` };
            }
            return execution.tests.find((item) => item.command === command) ?? { command, status: "not_run", evidence: "Executor returned no evidence for this test." };
        });
        ...
        for (const deliverable of handoff.deliverables) {
            ...
            let exists = false;
            try {
                exists = (await stat(resolve(handoff.workspace.root, deliverable.path))).isFile() || (await stat(resolve(handoff.workspace.root, deliverable.path))).isDirectory();
            }
            catch {
                exists = false;
            }
        ...
        if (execution.status !== "succeeded")
            findings.push({ severity: "blocker", message: `Executor status was ${execution.status}: ${execution.summary}` });
        if (execution.simulated)
            findings.push({ severity: "blocker", message: "Simulated executor evidence cannot receive PASS." });
        ...
        for (const item of tests.filter(({ status }) => status !== "passed"))
            findings.push({ severity: "major", message: `Required test was ${item.status}: ${item.command}` });
        for (const deliverable of missingDeliverables)
            findings.push({ severity: "major", message: `Missing deliverable: ${deliverable.path}`, path: deliverable.path });
        if (execution.proposedOperations.length)
            findings.push({ severity: "blocker", message: "Executor proposed new operations that require a fresh safety-gate evaluation." });
        ...
        const verdict = needsHumanReview ? "HUMAN_REVIEW" : passed ? "PASS" : "REVISE";
