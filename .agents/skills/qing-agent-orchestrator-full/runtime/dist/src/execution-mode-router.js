const CLI_INSTALL_GUIDE = "https://learn.chatgpt.com/docs/codex/cli";
const cliSignals = [
...
function stripNegatedCliSignals(text) {
    return text
...
export function detectCliReasonCodes(text) {
    const effective = stripNegatedCliSignals(text);
    return cliSignals.filter(({ pattern }) => pattern.test(effective)).map(({ code }) => code);
}
function recommendationBenefit(reasonCodes) {
    if (reasonCodes.includes("app-close-persistence"))
        return "CLI 可在桌面应用关闭后继续运行并保留过程状态。";
    if (reasonCodes.includes("scheduled-batch-unattended"))
        return "CLI 更适合定时、批量或无人值守执行。";
    if (reasonCodes.includes("machine-readable-control-plane"))
        return "CLI 可提供适合外部程序读取的状态、日志和取消接口。";
    if (reasonCodes.includes("script-or-ci"))
        return "CLI 更适合脚本、流水线和 CI 调用。";
    if (reasonCodes.includes("cli-only-model-or-environment"))
        return "所需模型、配置或环境只在已配置的 CLI 后端可用。";
    if (reasonCodes.includes("process-isolation-or-queue"))
        return "CLI 可提供独立进程或任务队列边界。";
    return "你明确要求使用 CLI 执行这个任务。";
}
function declinedLimitations(reasonCodes) {
    const messages = {
...
export async function respondToCliRecommendation(decision, response, inspector) {
    ...
    if (!inspector)
        throw new Error("Accepting a CLI recommendation requires a dependency inspector.");
