...
function plannerPrompt(task, workspace) {
    return [
...
function boundedPlannerFailure(stdout, stderr) {
...
function normalizePlannerOutput(parsed) {
    ...
    for (const criterion of criteria) {
        ...
        if (relayVerification &&
            ...
            relayVerification.kind === "event-count" &&
            relayVerification.field === null) {
            delete relayVerification.field;
        }
    }
}
export class CodexHandoffPlanner {
    ...
    async plan(task, requestedWorkspace) {
        if (!task.trim())
        ...
        if (this.options.modelSelection)
            args.push(...modelSelectionArgs(this.options.modelSelection));
        ...
        try {
            ...
            if (jsonlError)
                throw new Error(`Codex Planner protocol error: ${jsonlError}`);
...
export async function planLocally(task, requestedWorkspace, runtimeRoot) {
    if (!task.trim())
        throw new Error("Task text must not be empty.");
    ...
    if (["infrastructure", "external_action", "mixed"].includes(classification.category)) {
        throw new Error(`The conservative local Planner refuses '${classification.category}' tasks. Use the connected Planner or clarify and split the task.`);
    }
...
async function resolveSafeSchema(runtimeRoot, requestedPath) {
    ...
    if (relation.startsWith("..") || isAbsolute(relation))
        throw new Error("Handoff schema must be inside the Relay runtime.");
    return schema;
}
export async function saveHandoff(handoff, runtimeRoot, stateDirectory, requestedOutput) {
