export class NoHealthyModelCandidateError extends Error {
    fallbackAudit;
    constructor(message, fallbackAudit) {
        super(message);
        this.fallbackAudit = fallbackAudit;
...
export function validateModelCapability(candidate) {
    if (candidate.backend === "desktop-child" && candidate.profile !== null)
        return "Desktop child candidates cannot use CLI profiles.";
    if (candidate.backend === "desktop-child" && candidate.availability !== "host-advertised")
        return "Desktop child candidates must be explicitly host-advertised for this configuration snapshot.";
    if (candidate.backend === "codex-cli" && candidate.availability !== "entitlement-dependent")
...
function supports(candidate, request) {
...
function plannedCandidate(candidate, health) {
...
export function assessModelFallbackGate(plannedPair, actualPair, scope) {
    ...
    const reasons = [];
    if (!backendUnchanged)
        reasons.push("backend-changed");
    if (!scopeProofComplete)
        reasons.push("scope-proof-incomplete");
    if (scope?.operationsUnchanged === false)
        reasons.push("operations-changed");
    if (scope?.allowedPathsUnchanged === false)
        reasons.push("allowed-paths-changed");
    if (scope?.sandboxUnchanged === false)
        reasons.push("sandbox-changed");
    if (scope?.permissionsUnchanged === false)
        reasons.push("permissions-changed");
    if (scope?.effectsUnchanged === false)
        reasons.push("effects-changed");
    return { backendUnchanged, scopeProofComplete, ...normalizedScope, securityScopeUnchanged, requiresNewGate, reasons };
}
function explicitFallbackPlan(primary, candidates, health, request) {
...
function unavailableReason(candidate, health) {
    if (candidate.observedState === "host-advertised")
        return "Host-advertised candidate has not yet been rejected by a real spawn.";
    return health.get(candidate.candidateId)?.reason ?? `Observed state is ${candidate.observedState}.`;
}
function selectionFromPlan(plan, selectedIndex, role, complexityBand, attempts, fallbackReason) {
    ...
    if (gateAssessment.requiresNewGate)
        throw new ModelFallbackRequiresGateError(`Model substitution requires a fresh gate: ${gateAssessment.reasons.join(", ")}.`);
...
export function selectModelCandidate(candidates, health, request) {
    ...
    const tagScore = (candidate) => requestedTags.filter((tag) => candidate.tags.includes(tag)).length;
...
export function continueModelFallbackAfterRejection(selection, rejectedCandidateId, fallbackReason, scope) {
    if (!fallbackReason.trim())
        throw new Error("A concrete fallback reason is required after a model rejection.");
    if (selection.candidateId !== rejectedCandidateId)
        throw new Error(`Rejected candidate '${rejectedCandidateId}' is not the current actual candidate '${selection.candidateId}'.`);
    const currentIndex = selection.fallbackPlan.orderedCandidates.findIndex(({ candidateId }) => candidateId === rejectedCandidateId);
    if (currentIndex < 0)
        throw new NoHealthyModelCandidateError(`Rejected candidate '${rejectedCandidateId}' is absent from the explicit fallback plan.`);
    const attempts = selection.fallbackAudit.attempts.map((attempt) => attempt.outcome === "selected" && attempt.candidate.candidateId === rejectedCandidateId
        ? { ...attempt, outcome: "rejected", reason: fallbackReason }
        : attempt);
    ...
    for (let index = currentIndex + 1; index < selection.fallbackPlan.orderedCandidates.length; index += 1) {
        ...
        if (gateAssessment.requiresNewGate) {
            throw new ModelFallbackRequiresGateError(`Model substitution requires a fresh gate: ${gateAssessment.reasons.join(", ")}.`, { ...rejectedAudit, gateAssessment });
        }
    ...
    throw new NoHealthyModelCandidateError(`Explicit fallback chain exhausted after '${rejectedCandidateId}' was rejected; no healthy safe candidate remains.`, rejectedAudit);
}
