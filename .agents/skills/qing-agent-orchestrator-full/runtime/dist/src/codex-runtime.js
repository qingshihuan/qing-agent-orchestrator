...
export async function resolveCompleteWindowsRuntime(command, environment = process.env) {
    ...
    for (const directory of directories) {
        for (const name of names) {
            ...
            if (isWindowsAppsPath(candidate)) {
                failures.push("WindowsApps candidate rejected");
                continue;
            }
            if (!await isFile(join(dirname(candidate), sandboxHelperName))) {
                failures.push(`missing sandbox helper beside ${candidate}`);
                continue;
            }
    ...
    if (!explicit) {
        ...
        if (defaultRuntimeRoot && !isWindowsAppsPath(defaultRuntimeRoot)) {
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (isWindowsAppsPath(executable))
                        continue;
        ...
        if (defaultRuntimeRoot)
            failures.push(`no complete runtime in direct subdirectories of ${defaultRuntimeRoot}`);
        else
            failures.push("LOCALAPPDATA is not configured");
    }
...
function pathExtensions(environment) {
...
export async function resolveConfiguredExecutable(command, platform, environment) {
...
async function readPackageManifest(packageRoot) {
    try {
        const value = JSON.parse(await readFile(join(packageRoot, "codex-package.json"), "utf8"));
...
async function packageFromAncestor(executable) {
    let cursor = dirname(executable);
    for (let depth = 0; depth < 5; depth += 1) {
        if (await readPackageManifest(cursor))
            return cursor;
...
function defaultCodexHome(environment) {
    if (environment.CODEX_HOME)
        return resolve(environment.CODEX_HOME);
    if (environment.USERPROFILE)
        return resolve(environment.USERPROFILE, ".codex");
    return resolve(homedir(), ".codex");
}
async function matchingStandalonePackages(codexHome, version) {
    if (!version)
        return [];
...
async function standaloneProvenance(input, platform, environment, resolvedExecutable, packageRoot) {
    const manifest = await readPackageManifest(packageRoot);
    if (!manifest) {
...
function unknownProvenance(input, platform, resolvedExecutable, reason) {
    return {
        platform,
        configuredCommand: input.command,
        resolvedExecutable,
...
export async function inspectCodexRuntimeProvenance(input) {
