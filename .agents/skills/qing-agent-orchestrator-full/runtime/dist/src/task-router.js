...
export function createPendingDispatchHandoff(task, workspace, decision) {
    ...
    if (decision.signals.some((signal) => ["code-change", "content"].includes(signal))) {
        operations.push({ type: "write", target: "**/*", reason: "Create only the approved project-local deliverables.", risk: "medium" }, { type: "execute_tests", target: "project-local tests", reason: "Verify the approved deliverables locally.", risk: "low" });
    }
