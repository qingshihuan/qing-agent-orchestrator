...
const efforts = new Set<ModelReasoningEffort>(["none", "minimal", "low", "medium", "high", "xhigh", "max", "ultra"]);
const backends = new Set<ModelBackend>(["desktop-child", "codex-cli"]);
const availabilities = new Set<ModelAvailability>(["host-advertised", "entitlement-dependent"]);
const complexityBands = new Set<ComplexityBand>(["trivial", "normal", "complex", "high-risk"]);
...
function parseCandidates(value: unknown): ModelCandidate[] {
  ...
  const candidates = value.map((item, index): ModelCandidate => {
    ...
    if (!backends.has(item.backend as ModelBackend)) throw new Error(`modelRouting.candidates[${index}].backend is unsupported`);
    ...
    if (!availabilities.has(item.availability as ModelAvailability)) throw new Error(`modelRouting.candidates[${index}].availability is unsupported`);
    ...
    if (!candidateComplexityBands.every((band) => complexityBands.has(band as ComplexityBand)) || candidateComplexityBands.length === 0) throw new Error(`modelRouting.candidates[${index}].complexityBands contains an unsupported band`);
    const candidate: ModelCandidate = {
      id,
      backend: item.backend as ModelBackend,
      model,
      profile: profile as string | null,
      reasoningEffort: item.reasoningEffort as ModelReasoningEffort,
      availability: item.availability as ModelAvailability,
      roles: candidateRoles as ModelRole[],
      routes: routes as Array<"codex" | "hybrid">,
      categories: candidateCategories as TaskCategory[],
      complexityBands: candidateComplexityBands as ComplexityBand[],
    ...
    if (capabilityError) throw new Error(`modelRouting.candidates[${index}] capability mismatch: ${capabilityError}`);
    return candidate;
  });
  ...
  for (const candidate of candidates) {
    for (const fallback of candidate.fallbacks) {
      if (!idSet.has(fallback)) throw new Error(`Candidate '${candidate.id}' has dangling fallback '${fallback}'`);
      if (byId.get(fallback)?.backend !== candidate.backend) throw new Error(`Candidate '${candidate.id}' fallback '${fallback}' crosses model backends`);
    }
  }
...
export async function loadConfig(path?: string): Promise<RelayConfig> {
  ...
  if (orchestrationMode !== "adaptive" && orchestrationMode !== "full") throw new Error("orchestration.mode must be adaptive or full");
  const reviewerMode = orchestration.reviewerMode ?? defaultConfig.orchestration.reviewerMode;
  if (reviewerMode !== "risk-based") throw new Error("orchestration.reviewerMode must be risk-based");
  ...
  return {
    ...
    orchestration: {
      mode: orchestrationMode as OrchestrationMode,
      liteMaxChildren: numberSetting(orchestration.liteMaxChildren, defaultConfig.orchestration.liteMaxChildren, "orchestration.liteMaxChildren", 1, 1) as 1,
