...
export function routeTask(text: string, options: TaskRouteOptions = {}): TaskRouteDecision {
  ...
  if (deleteAction.test(effective)) signals.push("delete-action");
  ...
  if (privateNetworkAction.test(effective) && /访问|读取|连接|调用|access|read|connect|call/i.test(effective)) signals.push("private-network-action");
  if (scopeExpansionAction.test(effective)) signals.push("scope-expansion-action");
  if (databaseMigrationAction.test(effective)) signals.push("database-migration-action");
