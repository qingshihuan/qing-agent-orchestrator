...
import type { ExecutionModeDecision, ExecutionOwner, Handoff, OperationRequest, OrchestrationConfig, OrchestrationDecision, OrchestratorEdition, TaskCategory, TaskComplexityAnalysis, TaskRoute } from "./types.js";
...
function hasDeleteEffect(text: string): boolean {
  if (!deleteVerb.test(text)) return false;
  if (codeSymbolTarget.test(text) && !destructiveTarget.test(text) && !pathLikeTarget.test(text)) return false;
  return destructiveTarget.test(text) || pathLikeTarget.test(text) || /删除|清空|delete|purge|drop/i.test(text);
}

function hasDatabaseMigrationEffect(text: string): boolean {
  if (!databaseMigrationTarget.test(text)) return false;
...
export function routeTask(text: string, options: TaskRouteOptions = {}): TaskRouteDecision {
  ...
  if (contentCreation.test(effective)) signals.push("content");
  if (deploymentAction.test(effective)) signals.push("infrastructure");
  if (externalAction.test(effective)) signals.push("external-action");
  if (sequence.test(effective)) signals.push("plan-then-execute");
  if (orchestrationExecution.test(effective)) signals.push("orchestration-execution");
  if (hasDeleteEffect(effective)) signals.push("delete-action");
  if (gitPushAction.test(effective)) signals.push("git-push-action");
  if (purchaseAction.test(effective)) signals.push("purchase-action");
  if (globalWriteAction.test(effective) && globalWriteVerb.test(effective)) signals.push("global-write-action");
  if (secretAction.test(effective) && secretTarget.test(effective)) signals.push("secret-action");
  if (privateNetworkTarget.test(effective) && privateNetworkVerb.test(effective)) signals.push("private-network-action");
  if (scopeExpansionAction.test(effective)) signals.push("scope-expansion-action");
  if (hasDatabaseMigrationEffect(effective)) signals.push("database-migration-action");
  if (globalInstallAction.test(effective)) signals.push("global-install-action");
  ...
  if (actionableCliCondition) signals.push("cli-backend-condition");
...
export function createPendingDispatchHandoff(task: string, workspace: string, decision: TaskRouteDecision): Handoff {
  ...
  if (decision.signals.some((signal) => ["code-change", "content"].includes(signal))) {
    operations.push(
      { type: "write", target: "**/*", reason: "Create only the declared project-local deliverables.", risk: "medium" },
      { type: "execute_tests", target: "project-local tests", reason: "Verify the declared deliverables locally.", risk: "low" },
    );
  }
  ...
  if (decision.signals.includes("delete-action")) operations.push({ type: "delete", target: "deletion target requires Planner refinement", reason: "Deletion is a consequential effect and requires an exact target plus human approval.", risk: "high" });
  if (decision.signals.includes("git-push-action")) operations.push({ type: "git_push", target: "remote/ref requires Planner refinement", reason: "A remote Git push changes external state.", risk: "high" });
  if (decision.signals.includes("purchase-action")) operations.push({ type: "purchase", target: "purchase target and cost require Planner refinement", reason: "A purchase creates cost and an external side effect.", risk: "high" });
  if (decision.signals.includes("global-write-action")) operations.push({ type: "global_write", target: "global/system target requires Planner refinement", reason: "A global or system write changes state outside the project workspace.", risk: "high" });
  if (decision.signals.includes("secret-action")) operations.push({ type: "use_secret", target: "secret reference requires Planner refinement", reason: "Secret access requires an exact named reference and human approval.", risk: "high" });
  if (decision.signals.includes("private-network-action")) operations.push({ type: "network_access", target: "private/authenticated endpoint requires Planner refinement", reason: "Private or authenticated network access is not public read-only network_read.", risk: "high" });
  if (decision.signals.includes("scope-expansion-action")) operations.push({ type: "scope_expansion", target: "expanded scope requires Planner refinement", reason: "Material scope expansion needs fresh authority.", risk: "high" });
  if (decision.signals.includes("database-migration-action")) operations.push({ type: "database_migration", target: "database and migration plan require Planner refinement", reason: "A database migration can irreversibly change shared data.", risk: "critical" });
  if (decision.signals.includes("global-install-action")) operations.push({ type: "install_dependency", target: "global/system installation target requires Planner refinement", reason: "A global or system-level installation changes machine state.", risk: "high" });
  return {
    version: "1.0",
