...
import {
  ...
  type CodexRuntimeInspectionInput,
  type CodexRuntimeProvenance,
} from "../codex-runtime.js";
import { resolveSafeWorkspace } from "../workspace.js";
import type { OperationRequest, SandboxMode, WindowsSandboxMode } from "../types.js";
...
export interface CodexExecOptions {
  ...
  effectiveSandbox?: "read-only" | "workspace-write" | null;
  configuredSandbox?: "read-only" | "workspace-write" | null;
...
export type CodexRuntimeInspector = (input: CodexRuntimeInspectionInput) => Promise<CodexRuntimeProvenance>;
...
const mutatingOperationTypes = new Set<OperationRequest["type"]>([
  "write",
  "delete",
  "install_dependency",
  "network_access",
...
function operationIsReadOnly(operation: OperationRequest): boolean {
  if (operation.type === "read") return true;
  if (mutatingOperationTypes.has(operation.type)) return false;
...
export function isStrictReadOnlyDiagnostic(handoff: Handoff, sandbox: SandboxMode): boolean {
...
function validateJsonl(stdout: string): string | null {
...
export class CodexExecExecutor implements Executor {
  ...
  private async runDoctor(): Promise<CodexDoctorReport> {
    const errors: string[] = [];
    ...
    if (!authenticated) errors.push(processFailure("Codex CLI authentication unavailable", loginResult));
  ...
  async execute(handoff: Handoff, context: ExecutionContext): Promise<ExecutionResult> {
    ...
    const sandbox: SandboxMode = handoff.category === "advice" || handoff.category === "analysis" ? "read-only" : this.options.sandbox;
    let runtimeProvenance: CodexRuntimeProvenance;
    try {
  ...
  private async runDoctorFor(command: string, environment: NodeJS.ProcessEnv): Promise<CodexDoctorReport> {
    const run = (args: string[]) => this.runner.run({
      command,
      args,
      cwd: this.options.runtimeRoot,
      stdin: "",
