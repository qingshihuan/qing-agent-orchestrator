...
import type { ExecutionContext } from "./executors/executor.js";
import type { ProcessExitMetadata, ProcessStartMetadata } from "./process-runner.js";
...
function processStateFromExit(metadata: ProcessExitMetadata): ProcessState {
  if (metadata.cancelled) return "cancelled";
  if (metadata.timedOut) return "timed-out";
  if (metadata.exitCode === 0) return "exited";
  return "failed";
}

function terminalRunState(status: RelayStatus): { status: RunStatus; phase: RunPhase; processState: ProcessState } {
...
export class Relay {
  ...
  async run(handoff: Handoff, options: RelayOptions): Promise<RelayRunResult> {
    ...
    for (let iteration = 1; iteration <= maxIterations; iteration += 1) {
      ...
      const observabilityWrites: Promise<unknown>[] = [];
      let started: ProcessStartMetadata | null = null;
      let heartbeatTimer: ReturnType<typeof setInterval> | null = null;
      let detachObserver: (() => void) | null = null;
      let jsonlBuffer = "";
      let trustedCommands: TrustedCommandEvidence[] = [];
      ...
      const context: ExecutionContext = {
        ...
        onModelEvent: (type, value) => {
          if (!runHandle) return;
          const safeValue = JSON.parse(redact(JSON.stringify(value))) as Record<string, unknown>;
        ...
        onProcessHandle: (handle) => {
          ...
          detachObserver = handle.observe(({ stream, chunk }) => {
            ...
            for (const line of lines.filter(Boolean)) {
              ...
              } catch { summary = summary.replace(/(?:sk|sess)-[A-Za-z0-9_-]{8,}/g, "[REDACTED_TOKEN]"); }
        ...
        onSandboxPreflight: (value) => {
          ...
          if (typeof raw.ok === "boolean") payload.ok = raw.ok;
          if (typeof raw.effectiveSandbox === "string") payload.effectiveSandbox = raw.effectiveSandbox;
          if (typeof raw.requiresHumanReview === "boolean") payload.requiresHumanReview = raw.requiresHumanReview;
        ...
        onProcessStart: (metadata) => {
          ...
          heartbeatTimer = setInterval(() => {
            if (!runHandle || started === null) return;
      ...
      if (this.executor.name === "codex-exec" && handoff.testPlan.length > 0 && runHandle) {
        for (const command of handoff.testPlan) {
          ...
          const metadata: ProcessMetadata = { state: "running", pid: handle.pid, command: shell, args, cwd: handoff.workspace.root, startedAt, endedAt: null, exitCode: null, signal: null, cancelled: false, timedOut: false, outputLimitExceeded: false };
      ...
      let postExecutionGate: GateReport | undefined;

      if (execution.proposedOperations.length > 0) {
        postExecutionGate = evaluateSafetyGate(
          { ...handoff, id: `${handoff.id}-iteration-${iteration}`, requestedOperations: execution.proposedOperations },
          options.approvedGateIds,
        );
      }
      ...
      if (runHandle) {
        try {
          ...
          for (const evidence of collected) await runHandle.appendCriterionEvidence(evidence);
      ...
      if (!singleOwner) phaseState = recordIndependentReviewOutcome(phaseState, review.verdict);
      if (runHandle && !singleOwner) await runHandle.appendEvent("orchestration.review.outcome", "reviewing", iteration, "Independent review outcome committed.", { verdict: review.verdict, pendingIndependentReview: phaseState.pendingIndependentReview });
