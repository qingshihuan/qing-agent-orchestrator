...
The existing Relay heartbeat test now waits for persisted heartbeat events with a bounded deadline instead of assuming execution has begun after 50 ms. Its controlled executor remains open until the assertions have observed at least two heartbeats. Cleanup releases the executor and awaits the run before deleting its journal, including when an assertion fails. Assertions for execution phase, iteration, running state, ordered events, heartbeat payloads, terminal completion, and no heartbeats after exit are retained. This fixes the scheduling-dependent Linux/Node 18 failure without skipping tests or changing production heartbeat behavior.
