...
- 并发首次模型健康检查等待同一次缓存读取，等价候选共享探针并保留各自的审计 ID。
- 健康状态按当前模型、推理档位、profile、后端和角色指纹匹配，禁用或修改候选后不复用旧证明。
- 缓存写入串行化并使用唯一临时文件；磁盘缓存失败不改变实际探测结果，畸形记录忽略并重查。
- 失败的版本和目录发现可在后续显式检查恢复；取消或截断结果不再被当作健康证据。
- stdout/stderr 独立增量解码 UTF-8，修复跨块中文、emoji 和结尾残缺字符输出。
- 重复取消共享一次进程树终止流程；完成后释放原始输出缓冲和观察者引用，截断输出不保留超大底层 Buffer。
- 无效超时和输出预算在创建进程前拒绝；既有心跳测试改为等待持久化事件并在清理前等待退出。

### Packaging and verification

- 发布已合并 PR #12 的修复，重新生成完整版预编译运行时、技能 ZIP 和 SHA-256 清单；标准版内容不变。
- 测试从 142 项增加至 158 项；保留 Windows/Linux 与 Node 18/22 的类型检查、完整测试及打包一致性验证。
- 不改变模型候选、编排预算、效果审批、sandbox 或默认禁用真实执行的配置。
- 健康检查回归使用模拟执行器，未完成真实账户模型端到端验收；不声称整体速度或 Token 节省比例。

## 0.8.0 - 2026-08-26

### Added

- 阶段级重新分档策略：每个新用户回合及子任务创建、恢复、修订、审查前，只按剩余工作重判 Direct、Lite 或 Full。
- 未验收高风险产物的独立审查义务：中间阶段可降档，最终验收前自动恢复 Full 和独立 Reviewer。
- 事件驱动子任务协调决策：未变化状态只等待事件，停滞后只允许一次接管/替换决定，避免中断/恢复循环。

### Changed

- 自适应 Full 默认收紧为最多两个子任务和一次修订；显式、有界配置仍可在运行时最大值内提高预算。
- 标准版与完整版采用同一阶段路由、审查义务和协调规则；完整版 runtime 从测试后的源构建重新打包。
- Lite 的 `PARENT_VERIFICATION_REQUIRED` 结构化返回真实 Executor 证据及后置 gate（如有），且显式标记未调用 Reviewer。
- v0.7 `3/2` 兼容改为精确 Handoff ID 加内容 SHA-256 指纹；旧 ID allowlist 不再是可执行兼容入口。

### Evidence boundary

- 本版本以行为测试验证档位转换、审查义务保留、协调限流和预算默认值。真实模型 token 与端到端墙钟收益仍需持续用 connected 任务样本校准。
