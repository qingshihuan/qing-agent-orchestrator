...
test("dispatch exposes Direct, Lite, Full-ready, and gated Full without silent execution", async () => {
  ...
  try {
    ...
    assert.deepEqual(liteInvocation.spawnAgent, { model: "gpt-6-luna", reasoning_effort: "medium" });
    assert.equal(liteInvocation.executionOwner, "Codex");
    assert.equal(liteInvocation.parentModelUnchanged, true);
    assert.deepEqual(liteInvocation.fallbackPlan.orderedCandidates.map(({ candidateId }) => candidateId), ["desktop-luna-normal", "desktop-sol-normal"]);
